document.addEventListener("DOMContentLoaded", function (e) {
  ...
  window.onclick = function () {
    for (
      var e = document.getElementsByClassName("selectbox-options"), t = 0;
      t < e.length;
      t++
    )
  ...
  for (
    var s = document.getElementsByClassName("justselect"), n = 0;
    n < s.length;
    n++
  ) {
    (s[n].required = !0),
  ...
  for (
    var u = document.getElementsByClassName("justselect"), p = 0;
    p < u.length;
    p++
  ) {
  ...
  for (
    var v = document.getElementsByClassName("selectbox__label"), x = 0;
    x < v.length;
    x++
  )
      ...
      for (var a = 0; a < o.length; a++)
        o[a].onclick = function () {
          for (var e = 0; e < o.length; e++)
            e != a && o[e].setAttribute("class", "selectbox__option");
          ...
          leaderboardClick(this.dataset.value, false);
        };
    };
});

leaderboardClick = function (dataset, bool1) {
  datasetvar = dataset;
  if (topdown) bool1 = true;
  if (datasetvar == "kills") {
    updateLeaderboardView(datasetvar, bool1);
  } else if (datasetvar == "deaths") {
    updateLeaderboardView(datasetvar, bool1);
  } else if (datasetvar == "kd") {
    updateLeaderboardView(datasetvar, bool1);
  } else {
    $.post(
      "http://ffa/ffa:setcrosshair",
      JSON.stringify({
        crosshair: datasetvar,
      })
    );
  }
};
